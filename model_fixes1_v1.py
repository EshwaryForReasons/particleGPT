...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        inv_freq = 1.0 / (10000 ** (torch.arange(0, self.head_dim, 2).float() / self.head_dim))  # (head_dim//2,), theta = 10,000 from the original RoPE paper
    ...
    def _apply_rope(self, q, k, position_ids):
        """
        Apply RoPE using true absolute position_ids.
        ...
        if position_ids is None:
            position_ids = torch.arange(T, device=q.device, dtype=torch.long).unsqueeze(0)
        ...
        # cos_cached/sin_cached: (1, 1, block_size, head_dim)
        cos_base = self.cos_cached[0, 0]  # (block_size, head_dim)
        sin_base = self.sin_cached[0, 0]

        flat_pos = position_ids.reshape(-1)

        if flat_pos.max().item() >= cos_base.size(0):
            raise ValueError(
                f"RoPE position {flat_pos.max().item()} exceeds block_size/RoPE cache "
                f"{cos_base.size(0)}. Reduce max_new_tokens or increase block_size."
            )
    ...
    def forward(self, x, kv_cache=None, attn_mask=None, position_ids=None, use_cache=False):
        B, T, C = x.size()
        ...
        # OLD: Original rope block, doesn't work for inference
        # if False:
        ...
        # ===== RoPE =====
        # Training: position_ids = 0..T-1
        # Cached decoding: position_ids = cache_len
        ...
        # Training/full prefill with no explicit mask: causal=True.
        # Cached decode: causal=False, because the cache contains no future tokens.
...
class RELU2MLP(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        hidden_dim = int(getattr(config, "mlp_ratio", 4.0) * config.n_embd)
...
def build_mlp(config):
    mlp_type = getattr(config, "mlp_type", "relu2")
...
class GPTConfig:
    ...
    # MLP toggle/config
    mlp_type: str = "swiglu"  # "gelu", "relu2", "swiglu"
    mlp_ratio: float = 4.0
    # QK norm toggle
    qk_norm: bool = True
    # Embeddings norm toggle/config
    ...
    # @IMPORTANT: see above todo, it is unused for now
...
class GPT(nn.Module):
    ...
    def _embed_inputs(self, idx):
        """
        Shared embedding path for training, prefill, and decode.
        Keeps training/inference using the same input representation.
        """
    ...
    def mask_logits_for_next_token(self, logits, prev_token):
        """
        Generation-only grammar mask for your current format:
        ...
        allowed[
            is_start,
            dictionary.PDGID_OFFSET : dictionary.PDGID_OFFSET + dictionary.num_particles,
        ] = True
        ...
        allowed[
            is_pdgid,
            dictionary.PT_OFFSET : dictionary.PT_OFFSET + len(dictionary.pt_bins),
        ] = True
        ...
        allowed[
            is_pt,
            dictionary.ETA_OFFSET : dictionary.ETA_OFFSET + len(dictionary.eta_bins),
        ] = True
        ...
        allowed[
            is_eta,
            dictionary.PHI_OFFSET : dictionary.PHI_OFFSET + len(dictionary.phi_bins),
        ] = True
        ...
        # Fallback safety:
        # If a row somehow got no allowed tokens, do not mask that row.
    ...
    # OLD: GPT forward
    # def forward(self, idx, targets=None, kv_cache=None, attention_mask=None, position_ids=None, use_cache=False):
    #     device = idx.device
    #     b, t = idx.size()
    ...
    def forward(self, idx, targets=None, kv_cache=None, attention_mask=None, position_ids=None, use_cache=False):
        """
        General forward.
        Training/eval full-sequence path: ==> model(idx, targets)
        Prefill path:                     ==> model.forward_prefill(prompt)
        Decode path:                      ==> model.forward_decode(one_token, kv_cache)
        """
        device = idx.device
        b, t = idx.size()
        assert t <= self.config.block_size, (f"Cannot forward sequence of length {t}; block size is only {self.config.block_size}")
        ...
        if targets is not None:
            ...
            # Inference path: only need last-token logits.
    ...
    @torch.no_grad()
    def forward_prefill(self, idx, attention_mask=None):
        """
        Inference prefill.

        Processes the full prompt once and returns:
            logits:   logits for the last prompt token
            kv_cache: cache containing all prompt tokens
    ...
    @torch.no_grad()
    def forward_decode(self, idx, kv_cache):
        """
        Cached one-step decode.

        idx:      (B, 1)
        kv_cache: list of layer caches from forward_prefill or previous decode

        Returns:
            logits:   (B, 1, vocab_size)
            kv_cache: updated cache
        """
        assert idx.dim() == 2 and idx.size(1) == 1, (
            "forward_decode expects idx with shape (B, 1)"
        )
        assert kv_cache is not None and kv_cache[0] is not None, (
            "forward_decode requires an existing kv_cache from forward_prefill"
        )
    ...
    # OLD: generate, replaced with cache correct version below
    # @torch.no_grad()
    ...
    def generate(
        ...
        """
        Single-batch generation using the same cached path as batched sampling.
        ...
        prev_token = idx[:, -1]
        if grammar_mask:
            last_logits = self.mask_logits_for_next_token(last_logits, prev_token)
    ...
    def generate_batched_singleGPU(
        ...
        """
        Batched generation with correct KV-cache RoPE.

        Assumption for this simple fast path:
            all prompts in idx have the same non-padding length.

        This matches your current sampling script because you use:
        ...
        # Require fixed prompt length for the simple batched cache path.
        seq_lens = (idx != dictionary.padding_token).sum(dim=1)
        if not torch.all(seq_lens == seq_lens[0]):
            raise ValueError(
                "generate_batched_singleGPU currently expects all prompts to have the same "
                "non-padding length. Your sampling script uses event[:5], so this should be true."
            )
