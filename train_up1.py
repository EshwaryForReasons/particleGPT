...
import model_up1 as model
from model_up1 import GPTConfig, GPT
import pLogging
...
class pDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if device_type == 'cuda':
            x = x.to(conf.training.device)          # blocking, no pinning
            y = y.to(conf.training.device)
        else:
            x = x.to(conf.training.device)
            y = y.to(conf.training.device)
