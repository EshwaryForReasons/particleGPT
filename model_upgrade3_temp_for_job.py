...
#     Uses fp32 internally for the RMS calculation, then casts back to the
#     input dtype. This is usually safer for bf16/fp16 training.
#     """
...
#         # Do normalization math in fp32 for numerical stability.
#         x_float = x.float()
#         rms = torch.rsqrt(x_float.pow(2).mean(dim=-1, keepdim=True) + self.eps)
#         x_normed = x_float * rms

#         # Cast back to original dtype, then apply learned scale.
#         return (self.weight * x_normed).to(input_dtype)

# New RMSNorm for embedding normalization
class RMSNorm(nn.Module):
    """
    Root Mean Square Layer Normalization.
    ...
    def forward(self, x):
        ...
        # Do RMS calculation in fp32 for stability.
        x_float = x.float()
...
class GPTConfig:
    ...
    embedding_norm_init_scale: float = 1.0
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        # Newer RMS for embedding normalization transformer
        self.transformer = nn.ModuleDict(dict(
            wte = nn.Embedding(config.vocab_size, config.n_embd),
    ...
    def forward(self, idx, targets=None, kv_cache=None, attention_mask=None):
        ...
        # Pre embedding normalization version
