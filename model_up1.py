...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, kv_cache=None, attn_mask=None, position_ids=None):
        ...
        # # ===== RoPE rotation (applied per head; works the same for both MHA and GQA) =====
        # if position_ids is None:
        #     pos = torch.arange(T, device=x.device, dtype=torch.float).unsqueeze(0)  # (1, T)
        # else:
        ...
        # # Rotate Q, K
        # q = (q * cos[:, :, pos.long()]) + (rotate_half(q) * sin[:, :, pos.long()])
        # k = (k * cos[:, :, pos.long()]) + (rotate_half(k) * sin[:, :, pos.long()])
        # # RoPE does not touch V, so V remains as is.
        ...
        # ===== Expand KV heads to match Q heads (GQA core) =====
        if self.n_head_groups > 1:
            # k, v shapes: (B, n_kv_heads, T, head_dim)
            # Repeat each head n_head_groups times along the head dimension
            k = k.repeat_interleave(self.n_head_groups, dim=1)  # (B, n_head, T, head_dim)
            v = v.repeat_interleave(self.n_head_groups, dim=1)  # (B, n_head, T, head_dim)
...
class Block(nn.Module):
    ...
    def forward(self, x, kv_cache=None, attn_mask=None, position_ids=None):
        attn_out, new_kv_cache = self.attn(self.ln_1(x), kv_cache=kv_cache, attn_mask=attn_mask, position_ids=position_ids)
        x = x + attn_out
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            # wpe = nn.Embedding(config.block_size, config.n_embd), # wpe removed – RoPE handles positions
    ...
    def distance_sensitive_loss(self, logits, target_idxs, sigma=1.0):
        ...
        """
        ignore_index = self.dictionary.padding_token
    ...
    def forward(self, idx, targets=None, kv_cache=None, attention_mask=None):
        ...
        x = self.transformer.drop(tok_emb + type_emb)
        ...
        if targets is not None:
            ...
            if conf.training.loss_function == 'cross_entropy':
                ...
                loss = GPT.distance_sensitive_loss(logits.view(-1, logits.size(-1)), targets.view(-1), sigma=getattr(conf.training, 'loss_sigma', 1.0))
            else:
