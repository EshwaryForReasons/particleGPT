...
model_out_dir = script_dir / "trained_models" / conf.generic.model_name
preparation_dir = script_dir / 'data' / conf.generic.preparation_name
meta_path = preparation_dir / 'meta.pkl'
...
assert meta_path.exists(), "No meta file found, ensure data is prepared!"
...
assert conf.training.block_size != -1 or conf.training.context_events != -1
...
@dataclass
class DDPConfig:
    master_process: bool = True # this process will do logging, checkpointing etc.
...
def setup_ddp():
    ...
    ddp_config.ddp = True
    ddp_config.ddp_rank = int(os.environ['RANK'])
    ddp_config.ddp_local_rank = int(os.environ['LOCAL_RANK'])
    ddp_config.ddp_world_size = int(os.environ['WORLD_SIZE'])
    ddp_config.ddp_device = f'cuda:{ddp_config.ddp_local_rank}'
    ddp_config.seed_offset = ddp_config.ddp_rank 
    ddp_config.master_process = ddp_config.ddp_rank == 0
    
    conf.training.device = f'cuda:{ddp_config.ddp_local_rank}'
    torch.cuda.set_device(conf.training.device)
    # world_size number of processes will be training simultaneously, so we can scale
    # down the desired gradient accumulation iterations per process proportionally
    assert conf.training.gradient_accumulation_steps % ddp_config.ddp_world_size == 0
    conf.training.gradient_accumulation_steps //= ddp_config.ddp_world_size

# ddp = int(os.environ.get('RANK', -1)) != -1
# ddp_config = setup_ddp() if ddp else DDPConfig()

# # Disable logging for non-master processes
# if not ddp_config.master_process:
#     logger_idx = -1 
...
# train_dataset = pDataset(preparation_dir / 'train.bin')
# val_dataset = pDataset(preparation_dir / 'val.bin')

# # Data loader used for training.
# train_dataloader = DataLoader(
#     train_dataset,
#     batch_size=conf.training.batch_size,
#     shuffle=True if not ddp else False,  # DDP sets shuffle in DistributedSampler
#     drop_last=True,
...
best_ckpt_path = Path(model_out_dir, 'ckpt.pt')
running_ckpt_path = Path(model_out_dir, 'ckpt_running.pt')
...
if conf.training.init_from == 'scratch':
    ...
    pLogging.info(logger_idx, "Training progress", {"info": f"Resuming training from {model_out_dir}"})
...
class Trainer:
    def __init__(self, ddp_config):
        ...
        self.train_dataset = pDataset(preparation_dir / 'train.bin')
        self.val_dataset = pDataset(preparation_dir / 'val.bin')
        ...
        # Data loaders for loss estimations so the training loaders remain unaffected.
        self.le_train_dataloader = DataLoader(
            self.train_dataset,
            batch_size=conf.training.batch_size,
            shuffle=True if not ddp else False,  # DDP sets shuffle in DistributedSampler
            drop_last=True,
        ...
        self.le_val_dataloader = DataLoader(
            self.val_dataset,
            batch_size=conf.training.batch_size,
            shuffle=False,
            drop_last=True,
    ...
    def estimate_loss(self):
        ...
        out = {}
        model.eval()
        for split, dataloader in zip(['train', 'val'], [self.le_train_dataloader, self.le_val_dataloader]):
    ...
    def do_training(self):
        ...
        for num_micro_steps, (X, Y) in enumerate(self.train_dataloader):
            ...
            if b_grad_accum_steps_completed:
                ...
                if iter_num % conf.training.eval_interval == 0 and ddp_config.master_process:
                    self.save_checkpoint()
                
                if iter_num % conf.training.log_interval == 0 and ddp_config.master_process:
